...
def prepare_data():
	...
	df_1 = pd.read_csv('data/data/data_fe_2.csv')
...
if __name__ =="__main__":
	...
	for i in range(6):
		#lgb
		best_params_lgb = objective_lgb(X_train, Y_train.iloc[:, i], scoring[0])
		best_model_lgb = LGBMRegressor(**best_params_lgb)
		df_results_lgb = cv_train_test_model(best_model_lgb, X_train, Y_train.iloc[:, i], X_test, Y_test.iloc[:, i], cv, scoring[0])
		df_results_lgb = df_results_lgb.add_suffix('_lgb').set_axis([Y_train.iloc[:, i].name], axis=0)
		...
		study_xgb.optimize(lambda trial: objective_xgb(trial, X_train, Y_train.iloc[:, i], scoring[0]), n_trials=2)
		...
		df_results_xgb = cv_train_test_model(best_model_xgb, X_train, Y_train.iloc[:, i], X_test, Y_test.iloc[:, i], cv, scoring[0])
		df_results_xgb = df_results_xgb.add_suffix('_xgb').set_axis([Y_train.iloc[:, i].name], axis=0)
		#df_results_xgb.to_csv(data/str(i)+'_xgb_results.csv', index=False)

		#rf
		best_params_rf = objective_rf(X_train, Y_train.iloc[:, i])
		best_model_rf = RandomForestRegressor(**best_params_rf)
		df_results_rf = cv_train_test_model(best_model_rf, X_train, Y_train.iloc[:, i], X_test, Y_test.iloc[:, i], cv, scoring[0])
		df_results_rf = df_results_rf.add_suffix('_rf').set_axis([Y_train.iloc[:, i].name], axis=0)
	...
	df_results_all_property.to_csv('data/df_results_fe_2.csv', index=False)
