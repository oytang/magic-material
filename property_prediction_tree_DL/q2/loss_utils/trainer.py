...
def evaluate(model, valid_dataloader, criterion, device, use_cat):
    ...
    with torch.no_grad():
        ...
        all_pred = []
        loss_sum = 0
        for batch, (x, y, mask, TRUEY) in enumerate(valid_dataloader):
            ...
            y_pred = torch.where(mask, y_pred, mask.float())
            y = torch.where(mask, y, mask.float())
        ...
        all_pred = np.where( np.isnan(all_label), np.zeros_like(all_pred), all_pred)
        all_label = np.where( np.isnan(all_label), np.zeros_like(all_label), all_label)
...
def trainer(model, train_dataloader, valid_dataloader, device, max_epochs=2000, early_stop=3, use_cat=True):
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for batch, (x, y, mask, TRUEY) in enumerate(train_dataloader):
            x, y, mask, TRUEY = x.to(device), y.to(device), mask.to(device), TRUEY.to(device)
            if use_cat:
                y_pred = model(x[:,:-12], x[:,-12:].int())
            else:
                y_pred = model(x, None)
